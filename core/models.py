...
class MovRotativo(models.Model):
    ...
    def hora_total(self):
        if self.checkout is None and self.checkin is None:
            return math.ceil((self.checkout - self.checkin).total_seconds() / 3600) #transformando em hora

    def total(self):
        if self.valor_hora is None and self.hora_total is None:
